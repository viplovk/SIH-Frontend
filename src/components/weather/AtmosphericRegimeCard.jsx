    ...
    <div className="bg-[#111622] border border-white/[0.08] rounded-lg p-4 sm:p-5 relative">
      ...
      <div className="flex items-center justify-between pb-3 mb-3 border-b border-white/[0.06]">
        ...
        <div className="flex items-center gap-1.5 font-mono-tech text-xs">
          ...
          <span className="text-emerald-400 font-bold px-1.5 py-0.5 rounded bg-emerald-950/40 border border-emerald-500/30">
      ...
      <div className="bg-[#161d2d] border border-cyan-500/30 rounded p-3 mb-3">
        <div className="text-[10px] font-mono-tech text-cyan-400 uppercase tracking-wider mb-0.5">
          DETECTED ATMOSPHERIC CLUSTER
        </div>
        <div className="text-base sm:text-lg font-heading font-bold text-white tracking-wide">
          {selectedLocation.currentRegime}
        </div>
        <div className="text-xs text-slate-300 mt-1 font-mono-tech">
          Station: <strong className="text-cyan-300">{selectedLocation.name}</strong> ({selectedLocation.lat}°N, {selectedLocation.lon}°E)
        </div>
      </div>
      ...
      <div className="border-t border-white/[0.06] pt-3">
        <button
          ...
          className="w-full flex items-center justify-between text-xs font-mono-tech text-cyan-400 hover:text-cyan-300 py-1 cursor-pointer"
        >
        ...
        {isExpanded && (
          <div className="mt-2 p-3 bg-cyan-950/20 border border-cyan-500/20 rounded text-xs text-slate-300 leading-relaxed font-sans">
            <p className="font-medium text-cyan-200 mb-1 font-mono-tech">
              Automated Reasoning Explanation:
            </p>
            <p className="text-slate-300 text-[12px] leading-relaxed">
              {selectedLocation.weightRationale}
            </p>
            <div className="mt-2 pt-2 border-t border-cyan-500/10 flex flex-wrap gap-4 text-[11px] font-mono-tech text-slate-400">
              <span>NWP Allocation: <strong className="text-sky-400">{selectedLocation.modelWeights.nwp}%</strong></span>
              <span>AI Model A Allocation: <strong className="text-emerald-400">{selectedLocation.modelWeights.aiA}%</strong></span>
              <span>AI Model B Allocation: <strong className="text-amber-400">{selectedLocation.modelWeights.aiB}%</strong></span>
            </div>
          </div>
        )}
      </div>
