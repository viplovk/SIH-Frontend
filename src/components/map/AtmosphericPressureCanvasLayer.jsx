// Algoriot Real Continuous Atmospheric Pressure Field
// Evaluated from Open-Meteo Surface Pressure (hPa)
import React, { useEffect, useRef } from "react";
import { getIndiaBoundary, getPressureRgb } from "../../api/mapApi.js";

export function AtmosphericPressureCanvasLayer({
  map,
  stations = [],
  frameIndex = 0,
  visible = true,
  ...
  useEffect(() => {
    try {
      ...
      console.warn("Failed to load India boundary for pressure layer:", err);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    canvas.className = "algoriot-pressure-field-canvas";
    ...
    const render = () => {
      ...
      const blendRadius = Math.max(50, Math.hypot(dx, dy) * 1.4);

      for (let i = 0; i < stations.length; i++) {
        const s = stations[i];
        const frame = s.forecastSeries?.[frameIndex] || s.current;
        const pressure = frame?.pressure ?? 1010;
