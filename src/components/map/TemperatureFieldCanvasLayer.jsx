// Algoriot Real Continuous Temperature Field
// Evaluated from Open-Meteo Operational Forecasts (°C)
import React, { useEffect, useRef } from "react";
import { getIndiaBoundary, getTemperatureRgb } from "../../api/mapApi.js";

export function TemperatureFieldCanvasLayer({
  map,
  stations = [],
  frameIndex = 0,
  visible = true,
  ...
  useEffect(() => {
    try {
      ...
      console.warn("Failed to load India boundary for temperature layer:", err);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    canvas.className = "algoriot-temperature-field-canvas";
    ...
    const render = () => {
      ...
      // Render smooth continuous temperature field
      for (let i = 0; i < stations.length; i++) {
        const s = stations[i];
        const frame = s.forecastSeries?.[frameIndex] || s.current;
        const temp = frame?.temperature ?? 28.0;
